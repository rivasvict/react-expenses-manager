...
* A file's responsibilities should feel cohesive: someone opening it should be able to state what it is *for* in one sentence. This is **not** a one-function-per-file rule — helpers that serve a single concern belong together (`server/core/handlers/responses.ts` holds four related functions). It is a prompt to split when a file has accumulated several unrelated jobs, as `server/core/handlers.ts` had when it defined all three endpoints plus their shared helpers; each endpoint now lives in `server/core/handlers/` with its own colocated test, and `handlers.ts` is just the wiring.
