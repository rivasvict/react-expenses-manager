...
describe("joining a party", () => {
  ...
  it("rejects a wrong password without burning the invitation, then a retry succeeds", async () => {
    const code = seedInvitedTom("fam-pass");
    const session = server.loginAs(tom.email);
  ...
  it("rejects an already-used invitation with the exact copy", async () => {
    server.seedUser(jane);
    server.seedUser(tom);
    server.seedPartyWithMembers([jane.email]);
  ...
  it("defensively rejects a join while already in a party", async () => {
